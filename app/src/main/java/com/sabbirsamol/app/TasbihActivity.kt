...
class TasbihActivity : ComponentActivity() {
    ...
    private fun build() {
        ...
        val kaaba = LinearLayout(this).apply {
            ...
            addView(TextView(this@TasbihActivity).apply {
                ...
                setTextColor(Color.rgb(238, 210, 130)); setTypeface(null, Typeface.BOLD)
            })
        }
        ...
        nameText = TextView(this).apply {
            text = selected; textSize = 22f; setTextColor(Color.WHITE)
            setTypeface(null, Typeface.BOLD); gravity = Gravity.CENTER_VERTICAL
            setPadding(dp(12), 0, dp(4), 0)
        }
        root.addView(nameText, FrameLayout.LayoutParams(dp(230), dp(55)).apply {
            leftMargin = dp(8); topMargin = dp(143)
        })

        root.addView(Button(this).apply {
            text = "↻"; textSize = 25f; contentDescription = "রিসেট"
            setOnClickListener { resetCount() }
        }, FrameLayout.LayoutParams(dp(62), dp(52)).apply {
            gravity = Gravity.TOP or Gravity.END; rightMargin = dp(8); topMargin = dp(143)
        })

        countText = TextView(this).apply {
            textSize = 92f; setTextColor(Color.WHITE)
        ...
        titleRow.addView(TextView(this).apply {
            text = "জিকির লিস্ট"; textSize = 16f; setTextColor(Color.WHITE)
            setTypeface(null, Typeface.BOLD)
        }, LinearLayout.LayoutParams(0, dp(38), 1f))
        titleRow.addView(Button(this).apply {
            text = "⚙️"; setOnClickListener { customize() }
        ...
        for (z in zikirs()) {
            ...
            }, LinearLayout.LayoutParams(dp(145), dp(45)).apply { rightMargin = dp(6) })
        }
        ...
        root.addView(listBar, FrameLayout.LayoutParams(-1, dp(98)).apply {
            gravity = Gravity.BOTTOM; leftMargin = dp(8); rightMargin = dp(8); bottomMargin = dp(8)
        })

        // Full usable counter region. Header controls and bottom zikr list stay clickable.
        ...
        root.addView(touchLayer, FrameLayout.LayoutParams(-1, -1).apply {
            topMargin = dp(195)
            bottomMargin = dp(106)
            leftMargin = 0
            rightMargin = 0
    ...
    private fun resetCount() {
        AlertDialog.Builder(this).setTitle("রিসেট").setMessage("বর্তমান গণনা শূন্য করবেন?")
            .setNegativeButton("না", null).setPositiveButton("হ্যাঁ") { _, _ ->
                count = 0; prefs.edit().putInt("count", 0).apply(); refresh()
            }.show()
    }

    private fun customize() {
        val items = zikirs()
        AlertDialog.Builder(this).setTitle("📿 জিকির কাস্টমাইজ")
    ...
    private fun addZikir() {
        ...
        AlertDialog.Builder(this).setTitle("নতুন জিকির").setView(input)
            .setNegativeButton("বাতিল", null).setPositiveButton("যোগ") { _, _ ->
                val value = input.text.toString().trim()
                if (value.isNotEmpty()) {
                    val items = zikirs(); if (!items.contains(value)) { items.add(value); saveList(items) }; build()
                }
            }.show()
    }
    ...
    private fun editZikir(items: MutableList<String>, old: String) {
        ...
        AlertDialog.Builder(this).setTitle("জিকির পরিবর্তন").setView(input)
            .setNegativeButton("🗑 মুছুন") { _, _ ->
                items.remove(old); if (items.isEmpty()) items.add("সুবহানাল্লাহ"); saveList(items)
                if (selected == old) selected = items.first(); count = 0
                prefs.edit().putString("selected", selected).putInt("count", 0).apply(); build()
            }
            .setPositiveButton("💾 সংরক্ষণ") { _, _ ->
                val value = input.text.toString().trim()
                if (value.isNotEmpty()) {
                    val index = items.indexOf(old); if (index >= 0) items[index] = value; saveList(items)
                    if (selected == old) selected = value; prefs.edit().putString("selected", selected).apply(); build()
                }
            }.show()
    }
}
