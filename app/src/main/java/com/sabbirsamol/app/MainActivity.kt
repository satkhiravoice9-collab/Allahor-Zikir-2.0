...
class MainActivity : ComponentActivity(), LocationListener {
    ...
    private fun showHomeScreen() {
        ...
        // Seven-item fixed bottom navigation. Horizontal scrolling keeps all seven items in one row on small screens.
    ...
    private fun addBottomMenuButton(parent: LinearLayout, label: String, action: () -> Unit) {
        parent.addView(Button(this).apply {
            ...
            setPadding(4, 0, 4, 0)
            gravity = Gravity.CENTER
        }, LinearLayout.LayoutParams(112, 54).apply { setMargins(2, 0, 2, 0) })
    }
    ...
    private fun showTasbih() {
        ...
        box.addView(number); box.addView(add); box.addView(reset)
        ...
        reset.setOnClickListener { count = 0; number.text = "0"; prefs.edit().putInt("tasbih_count", 0).apply() }
        d.show()
    }
    ...
    private fun showLocationDialog() {
        AlertDialog.Builder(this).setTitle("লোকেশন নির্বাচন করুন").setItems(arrayOf("🛰️ GPS Auto Location", "🇧🇩 বাংলাদেশ ম্যানুয়াল নির্বাচন")) { _, w -> if (w == 0) enableGps() else showDivisionDialog() }.setNegativeButton("বাতিল", null).show()
    }
    ...
    private fun showDivisionDialog() {
        ...
        AlertDialog.Builder(this).setTitle("বিভাগ নির্বাচন করুন").setItems(a) { _, i -> selectedDivision = a[i]; showDistrictDialog() }.show()
    }
    ...
    private fun showDistrictDialog() {
        ...
        AlertDialog.Builder(this).setTitle("$selectedDivision - জেলা নির্বাচন করুন").setItems(a) { _, i -> selectedDistrict = a[i]; showThanaDialog() }.setNegativeButton("পেছনে") { _, _ -> showDivisionDialog() }.show()
    }

    private fun showThanaDialog() {
        val a = when (selectedDistrict) {
            "সাতক্ষীরা" -> arrayOf("শ্যামনগর", "সাতক্ষীরা সদর", "কালিগঞ্জ", "আশাশুনি", "কলারোয়া", "তালা", "দেবহাটা", "পাটকেলঘাটা")
            else -> arrayOf("সদর")
        }
        AlertDialog.Builder(this).setTitle("$selectedDistrict - উপজেলা নির্বাচন করুন").setItems(a) { _, i -> selectedThana = a[i]; locationText.text = "📍 $selectedThana, $selectedDistrict, $selectedDivision"; updatePrayerTimes() }.setNegativeButton("পেছনে") { _, _ -> showDistrictDialog() }.show()
    }

    private fun enableGps() {
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) { ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.ACCESS_FINE_LOCATION), 100); return }
        locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 1000L, 10f, this)
        Toast.makeText(this, "GPS লোকেশন চালু হয়েছে", Toast.LENGTH_SHORT).show()
    }
    ...
    private fun updatePrayerTimes() {
        ...
        prayerText.text = "ফজর: ০৪:৩০ AM\nযোহর: ১২:১৫ PM\nআসর: ০৪:৩০ PM\nমাগরিব: ০৬:৩০ PM\nএশা: ০৮:০০ PM"
        countdownText.text = "🕐 বর্তমান সময়: %02d:%02d".format(h, m)
    }

    private fun startCountdown() {
        handler.postDelayed(object : Runnable { override fun run() { updatePrayerTimes(); handler.postDelayed(this, 60000L) } }, 60000L)
    }
}
