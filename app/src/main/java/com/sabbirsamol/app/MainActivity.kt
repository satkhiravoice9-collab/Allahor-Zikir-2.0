...
class MainActivity : ComponentActivity(), LocationListener {
    ...
    private fun showHomeScreen() {
        ...
        val horizontal = HorizontalScrollView(this).apply { isHorizontalScrollBarEnabled = false; isFillViewport = false }
        val menu = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; setPadding(6, 4, 6, 6); setBackgroundColor(Color.WHITE) }
        ...
        horizontal.addView(menu)
        root.addView(horizontal, LinearLayout.LayoutParams(-1, 64))
        setContentView(root)
    }

    private fun addBottomMenuButton(parent: LinearLayout, label: String, action: () -> Unit) {
        parent.addView(Button(this).apply {
            text = label; textSize = 12f; setTextColor(Color.rgb(20, 83, 45)); setOnClickListener { action() }
            isAllCaps = false; minHeight = 0; minWidth = 0; setPadding(4, 0, 4, 0); gravity = Gravity.CENTER
    ...
    private fun showTasbih() {
        ...
        val add = Button(this).apply { text = "📿 গুনুন" }; val reset = Button(this).apply { text = "↩️ রিসেট" }
