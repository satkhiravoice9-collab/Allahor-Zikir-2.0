...
class MainActivity : ComponentActivity(), LocationListener {
    ...
    private fun showHomeScreen() {
        ...
        countdownText = tv("🕐 বর্তমান সময় হিসাব হচ্ছে...", 17f)
        content.addView(countdownText)

        val prayerTitle = tv("🕌 নামাজের সময়সূচী", 20f).apply {
            setTextColor(Color.rgb(20, 83, 45))
            setPadding(0, 10, 0, 4)
        }
        content.addView(prayerTitle)
    ...
    private fun addBottomMenuButton(parent: LinearLayout, label: String, action: () -> Unit) {
        val params = LinearLayout.LayoutParams(0, 56, 1f).apply {
            setMargins(1, 0, 1, 0)
        }
