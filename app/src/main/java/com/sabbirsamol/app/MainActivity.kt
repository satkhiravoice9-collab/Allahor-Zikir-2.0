...
class MainActivity : ComponentActivity(), LocationListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) { super.onCreate(savedInstanceState); locationManager=getSystemService(LOCATION_SERVICE) as LocationManager; showHomeScreen(); updatePrayerTimes(); startCountdown() }
    ...
    private fun showHomeScreen() {
        ...
        fun tv(t:String,s:Float)=TextView(this).apply{text=t;textSize=s;setTextColor(Color.rgb(20,83,45));gravity=Gravity.CENTER;setPadding(0,8,0,8)}
        content.addView(tv("🕌 Muslim Time",25f)); locationText=tv("📍 $selectedThana, $selectedDistrict, $selectedDivision",14f);content.addView(locationText)
        Button(this).apply{text="📍 লোকেশন পরিবর্তন";setOnClickListener{showLocationDialog()}}.also{content.addView(it)}
        countdownText=tv("পরবর্তী ওয়াক্ত হিসাব হচ্ছে...",18f);content.addView(countdownText);prayerText=tv("",17f).apply{setTextColor(Color.DKGRAY);gravity=Gravity.START};content.addView(prayerText)
        ...
        addBottomMenuButton(menu,"🏠\nহোম"){Toast.makeText(this,"আপনি হোম স্ক্রিনে আছেন",Toast.LENGTH_SHORT).show()};addBottomMenuButton(menu,"📿\nতাসবিহ"){showTasbih()};addBottomMenuButton(menu,"📚\nলাইব্রেরি"){showLibrary()};addBottomMenuButton(menu,"🤲\nমাসনুন আমল"){showMasnunAmal()};addBottomMenuButton(menu,"📝\nনোটপ্যাড"){showNotepad()};addBottomMenuButton(menu,"🔄\nরিফ্রেশ"){updatePrayerTimes();Toast.makeText(this,"নামাজের সময় আপডেট হয়েছে",Toast.LENGTH_SHORT).show()};addBottomMenuButton(menu,"ℹ️\nএবাউট"){showAboutDialog()}
    ...
    private fun showTasbih(){var count=prefs.getInt("tasbih_count",0);val box=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;gravity=Gravity.CENTER;setPadding(20,20,20,20)};val number=TextView(this).apply{text=count.toString();textSize=52f;gravity=Gravity.CENTER;setTextColor(Color.rgb(20,83,45))};val add=Button(this).apply{text="📿 গুনুন"};val reset=Button(this).apply{text="↩️ রিসেট"};box.addView(TextView(this).apply{text="তাসবিহ";textSize=22f;gravity=Gravity.CENTER});box.addView(number);box.addView(add);box.addView(reset);val d=AlertDialog.Builder(this).setView(box).setNegativeButton("বন্ধ",null).create();add.setOnClickListener{count++;number.text=count.toString();prefs.edit().putInt("tasbih_count",count).apply()};reset.setOnClickListener{count=0;number.text="0";prefs.edit().putInt("tasbih_count",0).apply()};d.show()}
